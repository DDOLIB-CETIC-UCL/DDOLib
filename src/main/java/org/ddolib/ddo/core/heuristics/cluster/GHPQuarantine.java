...
public class GHPQuarantine<T> implements ReductionStrategy<T> {
    ...
    public GHPQuarantine(StateDistance<T> distance, Relaxation<T> relaxation)  {
    ...
    public List<NodeSubProblem<T>>[] defineClusters(List<NodeSubProblem<T>> layer, int maxWidth) {
        ...
        List<NodeSubProblem<T>> nonQuarantined = new ArrayList<>();
        for (NodeSubProblem<T> node : layer) {
            if (distance.distanceWithBase(node.state) > 1) {
                nonQuarantined.add(node);
            }
            else pqClusters.add(new ClusterNode(Double.MAX_VALUE, List.of(node)));
        }

        pqClusters.add(new ClusterNode(0.0 , nonQuarantined));
