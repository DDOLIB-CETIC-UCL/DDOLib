...
public final class NoLayerAStarSolver<T> implements Solver {

    private final NoLayerProblem<T> problem;
    private final NoLayerFastLowerBound<T> lb;
    ...
    private final PriorityQueue<NoLayerSubProblem<T>> open = new PriorityQueue<>(
            Comparator.comparingDouble(NoLayerSubProblem<T>::f));
    private final NoLayerSubProblem<T> root;
    ...
    public NoLayerAStarSolver(NoLayerModel<T> model) {
    ...
    private NoLayerSubProblem<T> constructRoot(T state, double value) {
        return new NoLayerSubProblem<>(state, value, lb.fastLowerBound(state), new ArrayList<>());
    }
    ...
    public Solution minimize(Predicate<SearchStatistics> limit,
        ...
        while (!open.isEmpty()) {
            ...
            NoLayerSubProblem<T> sub = open.poll();
    ...
    private void addChildren(NoLayerSubProblem<T> subProblem, BiConsumer<int[], SearchStatistics> onSolution) {
        ...
        while (domain.hasNext()) {
            ...
            NoLayerSubProblem<T> newSub = new NoLayerSubProblem<>(newState, g, h, newPath);
