...
public class MispNoLayerProblem implements NoLayerProblem<MispNoLayerState> {

    public final BitSet[] neighbors;
    public final int[] weight;
    public final int nbVars;

    public MispNoLayerProblem(BitSet[] neighbors, int[] weight) {
    ...
    @Override
    public MispNoLayerState initialState() {
        BitSet remaining = new BitSet(nbVars);
        remaining.set(0, nbVars);
        return new MispNoLayerState(remaining, -1);
    }
    ...
    @Override
    public boolean isTarget(MispNoLayerState state) {
        return state.remainingNodes().isEmpty();
    }

    @Override
    public Iterator<Integer> domain(MispNoLayerState state) {
    ...
    @Override
    public MispNoLayerState transition(MispNoLayerState state, int label) {
        BitSet nextRemaining = (BitSet) state.remainingNodes().clone();
        nextRemaining.andNot(neighbors[label]);
        nextRemaining.clear(0, label + 1);
        return new MispNoLayerState(nextRemaining, label);
    }

    @Override
    public double transitionCost(MispNoLayerState state, int label) {
        return -weight[label];
    }
    ...
    public String toString() {
        return "MispNoLayerProblem(nbVars:" + nbVars + ")";
    }

    public static MispNoLayerProblem fromFile(String fname) throws java.io.IOException {
        ...
        return new MispNoLayerProblem(neighbor, weight.stream().mapToInt(x -> x).toArray());
    }
}
