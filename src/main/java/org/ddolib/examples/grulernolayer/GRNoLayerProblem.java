...
public class GRNoLayerProblem implements NoLayerProblem<GRNoLayerState> {

    public final int order;

    public GRNoLayerProblem(int order) {
        this.order = order;
    }

    @Override
    public GRNoLayerState initialState() {
        BitSet mark = new BitSet();
        mark.set(0);
        return new GRNoLayerState(mark, new BitSet(), 0);
    }
    ...
    @Override
    public boolean isTarget(GRNoLayerState state) {
        return state.getNumberOfMarks() == order;
    }

    @Override
    public Iterator<Integer> domain(GRNoLayerState state) {
    ...
    @Override
    public GRNoLayerState transition(GRNoLayerState state, int label) {
        ...
        return new GRNoLayerState(nextMarks, nextDistances, label);
    }

    @Override
    public double transitionCost(GRNoLayerState state, int label) {
    ...
    public String toString() {
        return "GRNoLayerProblem(order:" + order + ")";
    }
}
