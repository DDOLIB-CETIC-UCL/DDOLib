...
public class MaxCoverRelax implements Relaxation<MaxCoverState> {
    ...
    public MaxCoverState mergeStates(final Iterator<MaxCoverState> states) {
        ...
        int minNbSubsetAdded = state.nbSubsetAdded();
        int nbStatesMerged = 0;
        while (states.hasNext()) {
            state = states.next();
            intersectionCoveredItems.and(state.coveredItems());
            minNbSubsetAdded = Math.min(minNbSubsetAdded, state.nbSubsetAdded());
            nbStatesMerged++;
        }
        ...
        return new MaxCoverState(intersectionCoveredItems, minNbSubsetAdded);
    }
    /**
