package org.ddolib.examples.knapsacknolayer;

import org.ddolib.modeling.InvalidSolutionException;
import org.ddolib.modeling.nolayer.NoLayerProblem;

import java.util.Iterator;
import java.util.List;

public class KSNoLayerProblem implements NoLayerProblem<KSNoLayerState> {

    public final int[] profit;
    public final int[] weight;
    public final int capa;
    private final int nbItems;

    public KSNoLayerProblem(int[] profit, int[] weight, int capa) {
        this.profit = profit;
        this.weight = weight;
        this.capa = capa;
        this.nbItems = profit.length;
    }

    @Override
    public KSNoLayerState initialState() {
        return new KSNoLayerState(0, capa);
    }

    @Override
    public double initialValue() {
        return 0;
    }

    @Override
    public boolean isTarget(KSNoLayerState state) {
        return state.currentItem() >= nbItems;
    }

    @Override
    public Iterator<Integer> domain(KSNoLayerState state) {
        if (state.remainingCapacity() >= weight[state.currentItem()]) {
            return List.of(0, 1).iterator();
        } else {
            return List.of(0).iterator();
        }
    }

    @Override
    public KSNoLayerState transition(KSNoLayerState state, int label) {
        return new KSNoLayerState(
                state.currentItem() + 1,
                state.remainingCapacity() - label * weight[state.currentItem()]
        );
    }

    @Override
    public double transitionCost(KSNoLayerState state, int label) {
        return -label * profit[state.currentItem()];
    }

    @Override
    public double evaluate(int[] solution) throws InvalidSolutionException {
        if (solution.length != nbItems) {
            throw new InvalidSolutionException("Expected " + nbItems + " values, got " + solution.length);
        }
        int totalWeight = 0;
        int totalProfit = 0;
        for (int i = 0; i < nbItems; i++) {
            if (solution[i] == 1) {
                totalWeight += weight[i];
                totalProfit += profit[i];
            } else if (solution[i] != 0) {
                throw new InvalidSolutionException("Value must be 0 or 1, got " + solution[i] + " at index " + i);
            }
        }
        if (totalWeight > capa) {
            throw new InvalidSolutionException("Capacity exceeded: " + totalWeight + " > " + capa);
        }
        return -totalProfit;
    }

    public static KSNoLayerProblem fromFile(final String fname) throws java.io.IOException {
        boolean isFirst = true;
        int count = 0;
        int n = 0;
        int c = 0;
        int[] profit = new int[0];
        int[] weight = new int[0];
        try (final java.io.BufferedReader bf = new java.io.BufferedReader(new java.io.FileReader(fname))) {
            String line;
            while ((line = bf.readLine()) != null) {
                if (isFirst) {
                    isFirst = false;
                    String[] tokens = line.split("\\s");
                    n = Integer.parseInt(tokens[0]);
                    c = Integer.parseInt(tokens[1]);
                    profit = new int[n];
                    weight = new int[n];
                } else {
                    if (count < n) {
                        String[] tokens = line.split("\\s");
                        profit[count] = Integer.parseInt(tokens[0]);
                        weight[count] = Integer.parseInt(tokens[1]);
                        count++;
                    }
                }
            }
        }

        Integer[] items = new Integer[n];
        for (int i = 0; i < n; i++) {
            items[i] = i;
        }
        final int[] w = weight;
        final int[] p = profit;

        java.util.Arrays.sort(items, (o1, o2) -> {
            double ratio1 = (double) p[o1] / w[o1];
            double ratio2 = (double) p[o2] / w[o2];
            return Double.compare(ratio2, ratio1);
        });

        int[] sortedProfit = new int[n];
        int[] sortedWeight = new int[n];

        for (int i = 0; i < n; i++) {
            int j = items[i];
            sortedProfit[i] = profit[j];
            sortedWeight[i] = weight[j];
        }

        return new KSNoLayerProblem(sortedProfit, sortedWeight, c);
    }

    @Override
    public String toString() {
        return "KSNoLayerProblem(nbVars:" + nbItems + ")";
    }
}
