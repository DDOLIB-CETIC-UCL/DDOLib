...
public class BKSXPs {
    ...
    protected static BKSProblem[] loadInstances() throws IOException {
        int[] ns = new int[] {100, 200, 300};
        int[] ranges = new int[] {1000};
        BKSProblem.InstanceType[] types = BKSProblem.InstanceType.values();
        long[] seeds = new long[]{0, 1, 2, 3, 4, 5};
        BKSProblem[] problems = new BKSProblem[ns.length* ranges.length * types.length * seeds.length];
        int index = 0;
        for (int n: ns) {
            for (int r: ranges) {
                for (BKSProblem.InstanceType type: types) {
                    for (long seed: seeds) {
                        problems[index++] = new BKSProblem(n, r, type, seed);
                    }
                }
            }
        }
    ...
    private static DdoModel<Integer> getModel(BKSProblem problem,
                                                    int maxWidth,
    ...
    private static DdoModel<Integer> getModel(BKSProblem problem,
        ...
        return new DdoModel<>() {
            ...
            public StateDistance<Integer> stateDistance() {
                return new BKSDistance(problem);
            }
    ...
    private static void xpRelaxation() throws IOException {
        BKSProblem[] instances = loadInstances();
        FileWriter writer = new FileWriter("xps/relaxationsBKS.csv");
        writer.write("Instance;Optimal;ClusterStrat;MaxWidth;Seed;KmeansIter;HybridFactor;Incumbent;RunTime(ms)\n");

        for (BKSProblem problem : instances) {
    ...
    private static void xpRestriction() throws IOException {
        BKSProblem[] instances = loadInstances();
        FileWriter writer = new FileWriter("xps/restrictionBKS.csv");
        writer.write("Instance;Optimal;ClusterStrat;MaxWidth;Seed;KmeansIter;HybridFactor;Incumbent;RunTime(ms)\n");
