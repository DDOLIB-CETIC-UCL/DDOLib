...
public class MaxCoverXPs {
    ...
    private static MaxCoverProblem[] generateInstances() {
        int[] ns = {100, 150, 200};
        ...
        return new DdoModel<>() {
            ...
            public ReductionStrategy<MaxCoverState> relaxStrategy() {
                ...
                switch (clusterType) {
                    ...
                    case Hybrid -> strat = new Hybrid<>(new MaxCoverRanking(), stateDistance(), hybridFactor);
                }
                return strat;
            }
        ...
        for (MaxCoverProblem problem : instances) {
            for (int maxWidth = 10; maxWidth <= 100; maxWidth+=10) {
                for (ClusterType clusterType : new ClusterType[]{ClusterType.Cost, ClusterType.GHP, ClusterType.Hybrid}) {
                    ...
                    long[] ghpSeeds = clusterType != ClusterType.GHP ? new long[]{-1} : new long[]{465465, 546351, 87676};
