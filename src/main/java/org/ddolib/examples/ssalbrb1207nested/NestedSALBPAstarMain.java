...
/**
 * Nested Assembly Line Balancing Problem - A* Solver
 *
 * Solve the nested SALBP problem using A* search algorithm.
 *
 * Algorithm characteristics:
 *   A* is a heuristic search algorithm that uses evaluation function f(n) = g(n) + h(n)
 *   g(n) is the actual cost from start to current node
 *   h(n) is the heuristic estimate from current node to goal (lower bound)
 *   Guarantees finding optimal solution (if heuristic function is admissible)
 *   Suitable for exact solving of small to medium scale problems
 *
 * Program flow:
 ...
 *   Define A* model (problem, lower bound, dominance relation)
 *   Run A* algorithm
 *   Print detailed information and statistics of the solution
 */
public class NestedSALBPAstarMain {
     ...
     *             args[2]: Total number of robots (optional, defaults to 3)
...
//                : args[0];
        ...
        System.out.println("Nested Assembly Line Balancing Problem - A* Solver");
        System.out.println("=".repeat(80));
        System.out.println("Instance: " + instance);
        System.out.println("Cycle Time: " + cycleTime);
        System.out.println("Total Robots: " + totalRobots);
        System.out.println("Solver: A* Search");
        System.out.println("=".repeat(80));
        System.out.println();
        ...
        // Define A* model
        final Model<NestedSALBPState> model = new Model<>() {
            @Override
        ...
        // Run A* solver
        Solution solution = Solvers.minimizeAstar(model, (sol, searchStats) -> {
