...
/**
 * The {@code SimpleDominanceChecker} class implements a straightforward
 * dominance
 * management mechanism used in search algorithms based on Multi-valued Decision
 * Diagrams (MDDs),
 * such as DDO (Decision Diagram Optimization).
 * <p>
 * It maintains, for each depth level of the MDD, a collection of non-dominated
 * states,
 * according to a user-defined {@link Dominance} relation. When a new state is
 * explored,
 * this checker determines whether it is dominated by an existing state, or if
 * it dominates
 ...
 * </ul>
 * <p>
 * The goal is to avoid exploring redundant or inferior states during
 * optimization,
...
public class SimpleDominanceChecker<T> extends DominanceChecker<T> {
     ...
     * <p>
     * This class implements {@link Comparable} to allow ordering by descending
     * objective value,
     * and uses a deterministic unique ID as a tie-breaker to ensure consistent
     * ordering.
     * </p>
     */
    private class ValueState implements Comparable<ValueState> {
         ...
         * <p>
         * States are primarily compared by descending objective value, and in case of a
         * tie,
    ...
    /**
     * Updates the dominance front at a given depth level based on the provided
     * state.
     * <p>
     * The method performs the following steps:
     * </p>
     * <ol>
     * <li>Retrieves the dominance front corresponding to the given
     * {@code depth}.</li>
