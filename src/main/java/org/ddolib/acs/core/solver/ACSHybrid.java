...
public class ACSHybrid<T> implements Solver {
     ...
     */
    public ACSHybrid(AcsModel<T> model) {
    ...
    private ACSHybrid(AcsModel<T> model, StateAndDepth<T> rootKey) {
        this.problem = model.problem();
        this.varh = model.variableHeuristic();
        this.lb = model.lowerBound();
        this.dominance = model.dominance();
    ...
    private SubProblem<T> constructRoot(T state, double value, int depth) {
        ...
        double fastLB = lb.fastLowerBound(state, vars);
        double aggregateLB = Double.NEGATIVE_INFINITY;
        if (this.aggregate!= null) {
            T aggregateState = aggregate.compress(state);
            aggregateLB = aggregate.getLB(aggregateState,value);
        }

        if (aggregateLB > fastLB) {
            System.out.println(aggregateLB + ">" + fastLB);
            System.out.println(nullDecisions);
        }
        return new SubProblem<>(
                state,
                value,
                max(fastLB, aggregateLB),
                nullDecisions);
    }
    ...
    private void addChildren(SubProblem<T> subProblem) {
        ...
        while (domain.hasNext()) {
            ...
            double aggregateLB = Double.NEGATIVE_INFINITY;
            if (this.aggregate!= null) {
                T aggregateState = aggregate.compress(newState);
                aggregateLB = max(aggregateLB,aggregate.getLB(aggregateState, value));
            }
//            System.out.println("-----BEGIN-----");
//            System.out.println(newState + ":" + fastLowerBound);

//            if (aggregateLB > fastLowerBound) {
//                System.out.println(aggregateLB + ">" + fastLowerBound);
//                System.out.println(path);
//            }
//            System.out.println("-----END-----");
            SubProblem<T> newSub = new SubProblem<>(newState, value, max(fastLowerBound,aggregateLB), path);
//            if (newSub.f() < root.f() && aggregateLB==0){
//                System.out.println("ICI : "+ newState );
//            }
    ...
    private void checkAdmissibility() {
        ...
        DebugUtil.checkFlbAdmissibility(toCheck, model, key -> new ACSHybrid<>(model, key));
    }
